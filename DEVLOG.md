...
### 49. 3D ground truth: USGS 3DEP terrain + OSM buildings — improvement (Phase 1)
Turned the flat (x,y) world 2.5D. `data/terrain.py` pulls a USGS 3DEP DEM via `py3dep` (cached to `data_cache/sf_dem.tif`), reprojects road nodes into the DEM's native CRS (it serves EPSG:5070 Albers, not lon/lat — the bug that first made every sample NaN), bilinearly samples a per-node `z` (downtown SF: 4–107 m), and derives a per-edge grade clipped to ±0.35 (11 edges >15% — the real SF hills). `data/buildings.py` pulls 2,491 OSM footprints via OSMnx (cached to parquet), estimates heights (`height` → `levels×3` → 8 m default; up to 260 m), and flattens them into 29,029 occluder wall segments in the metric frame. Both degrade gracefully to zeros/empty when offline so the 2D path is untouched. `RoadNetwork` now carries `node_z`/`edge_grade`/`bbox`; `load_road_network_3d()` composes graph+terrain+buildings. Gate artifact: `scripts/preview_terrain.py` → hillshade + footprints PNG.

### 50. 3D env: grade dynamics + z draping + building occlusion — improvement (Phases 2–3)
`RoutePool` and `Env` now carry `z`/`grade` per waypoint; `State` carries draped `z`. Dynamics gained a cheap grade term (`net accel = command − g·grade`) so uphills sap acceleration/top-speed and downhills lengthen braking (closed-form tests: flat ground still obeys s=½at², uphill ends slower for the same action). New `env/perception.py` does vectorized JAX ray-vs-wall occlusion: a neighbor hidden behind a building is dropped from the K-nearest obs (the policy genuinely can't see through buildings), plus a nearest-building distance feature. `obs_dim` 26→28 (added grade + building distance) — invalidates old checkpoints (expected; retrain). Tests verify occlusion vs a brute-force reference.

### 51. CMDP reframe + reproducible trace + deterministic verifier — architectural change (Phase 4)
Split "what we optimize" from "what we forbid": reward is now efficiency (−Δtravel_time + arrival bonus, light progress shaping), and crash/off-road/rule are a dense per-agent **constraint cost channel** in `info["cost"]` (the PPO-Lagrangian machinery's input), never an all-or-nothing run discard. `env/trace.py` writes a replayable JSONL trace (Manifest with a config-hash + per-car Timeline + a scripted-driver rollout so the demo drives without a checkpoint). `eval/verifier.py` is a **pure function over the trace** — geometric predicates only (crash = footprint overlap, off-road = lateral deviation/polygon, rule = over the speed limit, arrived = reached destination), so same trace → same verdict, immune to GPU/float non-determinism. Tests cover determinism, planted crash/off-road/speeding, JSONL round-trip, and agreement with the env's online info.

### 52. CesiumJS 3D demo: meshed cars on real 3D SF — improvement (Phases 5–6)
`demo/export_cesium.py` runs the whole pipeline (3D ground truth → rollout → trace → verify → export) and packs per-car (lon,lat,height,heading,speed,crash) + the verifier summary into `trajectories.json` (no geometry baked in — Cesium streams terrain + OSM buildings from ion). `demo/cesium/` renders box-meshed cars that drape onto Cesium World Terrain (heights sampled from the live terrain provider so they climb the actual hills), oriented by velocity, colored by speed, with a verifier-driven HUD and a graceful no-token fallback. `scripts/build_3d_demo.py` is the one-command end-to-end; `scripts/smoke_env.py` now also exercises the 3D env (grade + occlusion under jit). Deferred per the plan: Isaac PhysX execution + Cosmos/Antim (scaffolding untouched).
